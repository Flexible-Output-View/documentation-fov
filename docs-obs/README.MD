...
* **Unified Container Transport:** All parallel encoded video and audio streams are packaged sequentially into a singular MPEG-TS (MPEG Transport Stream) container utilizing specialized Program Identifiers (PIDs) before being pushed over the network.
...
3.  **PID Index Sequencing:** Within the MPEG-TS multiplexer loop, audio track indexing begins exactly where video track array iteration terminates, ensuring flawless track allocation inside the transport stream:
...
> [!Warning] Hardware Resource Constraints
...
> [!Note] Scene Management Limitations
...
* In the Web GUI, these tracks will be instantly separated, allowing viewers to mute, isolate, or adjust volumes of game audio vs. vocal audio completely independently.
